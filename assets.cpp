...
/***********************************************ASSETS FOR ARCHERY PROJECT**************************************************/
	...
	Bow genBow(const Point &p)
	{
		...
		bow.center.x = p.x;
		bow.center.y = p.y;
		bow.radius = 100;                     // this needs to be a parameter - this still needs to be decoupled from system resolution. Please consider sending it as an artist controllable parameter
        	setcolor(WHITE);
	...
	Arrow genArrow(const Point &p, int &size)  //body to size ratio is 10:1
	{
                                    	     //restraints to be included to make sure the object is within the window limits
        	Arrow aro;
        	aro.p = p;
        	aro.size = size;
        	return aro;                 //returning the Arrow object generated by the function
	}
	void drawArrow(Arrow aro)
	{
        	setlinestyle(0 , 0 , 1);
        	setcolor(WHITE);
        	moveto(aro.p.x , aro.p.y);                                   //https://www.cs.colorado.edu/~main/bgi/doc/moveto.html
        	linerel(10 * aro.size, 0);     //linerel(int dx,int dy)      //https://www.cs.colorado.edu/~main/bgi/doc/linerel.html

        	//head part of the arrow

        	linerel(-2 * aro.size , -1 * aro.size + 1);
        	linerel(0 , 2 * aro.size - 1);
        	linerel(2 * aro.size , -1 * aro.size);

        	//tail part of the arrow

        	moveto(aro.p.x + 15 , aro.p.y);
        	aro.size = aro.size - 3;
