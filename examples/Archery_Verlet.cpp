...
int main()                    //Test Arrow and Bow
{
	...
	while (1)
	{
		...
		nextPointArrow.x= arrow.locus.x + 4;
		nextPointArrow.y = arrow.locus.y;
			...
			*/
			...
			if (ismouseclick(WM_LBUTTONDOWN))                    //occurrence of event is checked
			{
				...
				while (nextPointArrow.x + (8 * arrow.size) <= target.vert.src.x)
				{
					...
					drawTarget(target);*/
					nextPositionArrow = getNextPositionVerlet(arrow.locus, nextPointArrow, acceleration, stepSize, theta);
					genBow(bow);
					drawBow(bow, FALSE);
